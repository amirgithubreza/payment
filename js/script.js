...
/* ── OFFLINE VALIDATION (تاریخ و ساعت) ────── */
...
function goToStep(step) {
    ...
    if (step === 4) {
        ...
        if (S.method === 'offline') {
            ...
            if (!isValid) {
                showToast('لطفاً تمام فیلدهای اجباری (تاریخ، ساعت، فیش) را تکمیل کنید.', 'error');
                return;
            }
        }
    }
...
document.querySelectorAll('.step-header .cstep').forEach((el) => {
    el.addEventListener('click', function () {
        ...
        if (step === 1 || (step > 1 && S.isStep1Valid)) {
            ...
            showToast('لطفاً ابتدا اطلاعات مرحله ۱ را کامل کنید.', 'error');
        }
    });
});
